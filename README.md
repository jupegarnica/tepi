...
* `-p` `--threads` `--parallelization`  Set the maximum number of test blocks tepi can run at once. Defaults to 1.
...
`tepi -p 3`
> Run up to 3 ready test blocks in parallel. Same as `tepi --threads 3`.
